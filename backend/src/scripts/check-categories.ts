...
async function checkCategories() {
  try {
    ...
    columns.recordset.forEach((row: any) => {
    ...
    spExists.recordset.forEach((row: any) => {
      console.log(`  ${row.name}`);
    });

    // If sp_Categories_Create doesn't exist, create it
    if (!spExists.recordset.some((r: any) => r.name === 'sp_Categories_Create')) {
    ...
    if (!spExists.recordset.some((r: any) => r.name === 'sp_Categories_GetByStore')) {
    ...
    try {
      const result = await pool.request()
        ...
        .input('storeId', sql.UniqueIdentifier, pokemonStore)
        .input('name', sql.NVarChar, 'Test Category')
