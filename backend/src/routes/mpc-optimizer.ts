...
import { mpcOptimizer } from '../services/mpc-inventory-optimizer';
import sql from 'mssql';
import { getConnection } from '../db';

interface MPCOptimizeRequest extends AuthRequest {
  body: {
    productId: string;
    customParams?: {
...
interface BatchOptimizeRequest extends AuthRequest {
  body: {
    productIds?: string[];
    limit?: number;
  };
 ...
 */
router.post('/optimize', async (req: MPCOptimizeRequest, res: Response) => {
  try {
    ...
    const historicalSales = salesResult.recordset.map((r: any) => r.totalQuantity);

    // Dự báo nhu cầu
    const demandForecast = mpcOptimizer.forecastDemand(historicalSales, customParams?.predictionHorizon);

    // Tối ưu hóa quyết định
    const optimalDecision = mpcOptimizer.optimize(
      {
    ...
    const avgDailyDemand = historicalSales.reduce((a: number, b: number) => a + b, 0) / historicalSales.length;
    const variance = historicalSales.reduce((sum: number, val: number) => sum + Math.pow(val - avgDailyDemand, 2), 0) / historicalSales.length;
    const demandStdDev = Math.sqrt(variance);

    const reorderPoint = mpcOptimizer.calculateReorderPoint(avgDailyDemand, demandStdDev);
    const eoq = mpcOptimizer.calculateEOQ(avgDailyDemand * 365, product.averageCost);
 ...
 */
router.post('/batch-optimize', async (req: BatchOptimizeRequest, res: Response) => {
  try {
    const storeId = req.storeId!;
    ...
    if (productIds && productIds.length > 0) {
      query += ` AND p.id IN (${productIds.map((_: any, i: number) => `@productId${i}`).join(',')})`;
    } else {
    ...
    for (const product of products) {
      ...
      const demandForecast = mpcOptimizer.forecastDemand(historicalSales);
      const optimalDecision = mpcOptimizer.optimize(
        {
          productId: product.id,
          currentStock: product.currentStock,
 ...
 */
router.get('/abc-analysis', async (req: AuthRequest, res: Response) => {
  try {
    const storeId = req.storeId!;
    const pool = await getConnection();
    ...
    const products = result.recordset.map((r: any) => ({
      id: r.id,
      name: r.name,
      revenue: r.revenue,
    ...
    const classification = mpcOptimizer.classifyABC(products);
    ...
    const summary = {
      A: classified.filter((p: any) => p.class === 'A').length,
      B: classified.filter((p: any) => p.class === 'B').length,
      C: classified.filter((p: any) => p.class === 'C').length,
    };
