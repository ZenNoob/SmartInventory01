...
export class VoucherService {
  ...
  async validateVoucher(
    ...
    if (customerId && voucher.usagePerCustomer) {
      const usageCount = await queryOne(
        `SELECT COUNT(*) as count FROM VoucherUsage 
         WHERE voucher_id = @voucherId AND customer_id = @customerId`,
        { voucherId: voucher.id, customerId }
      );

      if (usageCount && (usageCount as any).count >= voucher.usagePerCustomer) {
  ...
  async recordUsage(
    ...
    await query(
      `INSERT INTO VoucherUsage (id, voucher_id, customer_id, sale_id, discount_amount)
       VALUES (NEWID(), @voucherId, @customerId, @saleId, @discountAmount)`,
      { voucherId, customerId, saleId, discountAmount }
    );

    await query(
      `UPDATE Vouchers SET usage_count = usage_count + 1 WHERE id = @voucherId`,
      { voucherId }
    );
  }
   ...
   */
  generateCode(prefix: string = 'SALE', length: number = 8): string {
