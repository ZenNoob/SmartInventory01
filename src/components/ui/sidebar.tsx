...
>(
  (
    ...
    // We add a state so that we can do data-sidebar-state="expanded" or "collapsed".
...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn("group/sidebar peer hidden md:block text-card-foreground sticky top-0 h-dvh", className)}
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 p-3 overflow-auto group-data-[collapsible=icon]:items-center",
        className
      )}
      {...props}
    />
